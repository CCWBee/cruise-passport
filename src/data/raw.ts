...
// The label the ship prints where it differs from the deck number. Added, not edited: this fact
// exists today only as two inline conditionals in FilterPanel.tsx (lines 71 and 79), so Ship and
// Stats say "Deck 15" while the filter panel says "Deck 15/16". model.ts's deckLabel() reads it,
// and a sailing with no labels prints the number.
export const DECK_LABELS: Record<number, string> = { 15: "15/16" };

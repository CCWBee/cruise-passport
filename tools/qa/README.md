...
| `webkit-shots.mjs` | Headless WebKit (Playwright, installed in a scratch folder, never the project; its header has the steps). The glass surfaces at 390 by 844, scale 3, three hours, with the computed clip of each glass layer. It prints first whether this build paints backdrop-filter: the Windows build does not, so a clean render proves shape and clip, not the blur. Only inside `capped.ps1`, with options in `WK_BASE`, `WK_HOURS` and `WK_OUT` and the install in `PW_DIR` and `PLAYWRIGHT_BROWSERS_PATH`. |

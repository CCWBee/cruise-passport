...
- **Earned**: the heading carries "6 of 18" at the right, and nothing while none is earned. The
  ...
  bronze, and within a tier the order of `BADGES`, except that a count ladder's higher rung comes
  first, so Fifty leads Twenty Five and is the best medal Home's tray names), three to a row at 390 and two at 320, each with
...
| `medalGroups()`, `progressOf()`, `remainder()`, `struckLine()`, `unstruckLine()` | `src/features/badges/medals.ts` | the case's arithmetic: earned in tier order (a count ladder's higher rung first), in reach nearest first, the rest locked; what is left in the badge's own unit; the medal sheet's dated line. Home's tray and You's small case read it too, so the three cannot count differently. A module of its own, so `Badges.tsx` exports only its component |
